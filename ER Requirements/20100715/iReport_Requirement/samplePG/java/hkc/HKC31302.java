 ...
 ********************************************************************************
 ================================================================================
   ホクコウ共通プログラム
 ================================================================================
   Program Number  : HKC31302.java
   Program Name    : 銀行支店データファイルの形式チェック
 ...
 ********************************************************************************
 */
public class HKC31302 implements IFile {
	/* +-----------------------------------------------------------------------+ */
	/* | 読込用のBUffer定義                                                    | */
	/* +-----------------------------------------------------------------------+ */
	...
	public void doUploading(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			while ((i = _FileInStream.readLine(_FileByteData, 0, 1024)) != -1){
				...
				if (intCnt != 1){
					if (strBuf.length() != 24){
						_strInfo = "銀行支店データの桁数が違います。";
						break;
					}
					strWk = strBuf.substring(0, 7);
					if (_check_number(strWk) == false){
						_strInfo = "銀行支店コードに数字以外の文字が含まれています。";
						break;
					}
				}
			}
			if (intCnt < 2){
				_strInfo = "銀行支店データの内容が０件です。";
			}
