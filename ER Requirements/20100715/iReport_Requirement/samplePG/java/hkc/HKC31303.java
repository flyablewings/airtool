 ...
 ********************************************************************************
 ================================================================================
   ホクコウ共通プログラム
 ================================================================================
   Program Number  : HKC31303.java
   Program Name    : 金融機関データ受入(銀行)
 ...
 ********************************************************************************
 */
public class HKC31303 implements IFile {
	/* +-----------------------------------------------------------------------+ */
	/* | 読込用のBUffer定義                                                    | */
	/* +-----------------------------------------------------------------------+ */
		...
		try{
			...
			while ((i = _FileInStream.readLine(_FileByteData, 0, 1024)) != -1){
				...
				if (intCnt != 1){
					...
					if (_read_A12_Bank() == false){
						_insert_A12_Bank();            // 銀行表への新規書込み
					}else{
						_update_A12_Bank();	           // 銀行表の書替
					}
				}
			}
	...
	*/	
	...
	private void _save_Rec(String strBuf) throws Exception {
		...
		try{
			...
			_strBuf[1] = strBuf.substring(4, 19);
		} catch (Exception e) {
			throw e ;
		} finally {
		}
	...
	/* +-----------------------------------------------------------------------+ */
	private  boolean _read_A12_Bank() throws Exception {
	...
	/* | 銀行表への書込み処理                                                  | */
	/* +-----------------------------------------------------------------------+ */
	private void _insert_A12_Bank() throws Exception {
		DB_Statement dbs = null;
		StringBuffer stbSql = new StringBuffer();
	...
	/* | 銀行表の書替処理　　　　　　　　　　　　　　                          | */
	/* +-----------------------------------------------------------------------+ */
	private void _update_A12_Bank() throws Exception {
		DB_Statement dbs = null;
		StringBuffer stbSql = new StringBuffer();
		try {
			...
			stbSql.append("\n"+"     A12_BankKN  = '" + DB_CtrlChar.edit(_strBuf[1]) + "' ");
			stbSql.append("\n"+"WHERE ");
			stbSql.append("\n"+"     A12_BankCD = " + _strBuf[0] + " ");
			stbSql.append("\n"+";");
