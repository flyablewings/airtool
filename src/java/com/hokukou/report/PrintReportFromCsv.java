...
================================================================================
	ホクコウ共通ライブラリ In Java
================================================================================
	Program Number  : PrintReportFromCsv.java
	Program Name    : 印刷用のCSVファイル管理
...
===< Update History >===========================================================
	2008/06/24  : 新規作成
	2008/07/06  : 修正												= 張飛
	2008/08/29  : Linuxサーバー下での印刷での文字化けを修正         = H.Fujimoto
	2008/08/30  : Excel保存で文字化けする問題を修正         		= H.Fujimoto
	2008/11/01  : 改行への対応        								= H.Fujimoto
	2009/02/25  : 印刷データにﾀﾞﾌﾞﾙｺｰﾃｰｼｮﾝ(")がある場合に印刷できない問題を修正 = T.OHORI
================================================================================
********************************************************************************
*/

public class PrintReportFromCsv {
	 ...
	 * コンストラクター
	 */
	public PrintReportFromCsv(String[] strColName) {
	...
	public void setData(HashMap hsmRow) throws IOException {
		...
		for (int intIdx = 0;intIdx < _strColName.length; intIdx++) {
			...
			/***** 2009/02/25 UPDATE START *****/
			// 印刷データをﾀﾞﾌﾞﾙｺｰﾃｰｼｮﾝでくくる。
			stbSql.append("\"");
			if (hsmRow.get(_strColName[intIdx]) != null) {
				strBuf = hsmRow.get(_strColName[intIdx]).toString();
				// ﾀﾞﾌﾞﾙｺｰﾃｰｼｮﾝがあった場合、ﾀﾞﾌﾞﾙｺｰﾃｰｼｮﾝを二重にする。
				strBuf = strBuf.replace("\"", "\"\"");
				// 改行コードを整える				 
				strBuf = strBuf.replace("\n\r", "\n");
				strBuf = strBuf.replace("\r", "\n");
				stbSql.append(strBuf);
			}
			stbSql.append("\"");
			/***** 2009/02/25 UPDATE E N D *****/
		}
